...
@st.cache_data
def load_data():
    dealer_stock = pd.read_excel("Dealer_Stock_Summary_Clean.xlsx")
    lease_xls = pd.ExcelFile("H202_BulletinCE.xlsx")
    return dealer_stock, lease_xls

dealer_stock, lease_xls = load_data()
...
if vin_input:
    ...
    if vin_input in dealer_stock.index:
        ...
        df_table = pd.read_excel(lease_xls, sheet_name="Table 17")
